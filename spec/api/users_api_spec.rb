...
describe 'User API' do
  ...
  it 'should return and confirm user when providing existing confirmation_token' do
    ...
    response.status.should == 200
    json['confirmation_token'].should be_nil
    json['email'].should == @user.email
  end
  ...
  it 'should allow registering new user and not require API token', register: true do
    ...
    response.status.should == 200
    @json['token'].should == ApiKey.last.token
  end
  ...
  it 'should validate user creation and return user with validation errors', register: true do
    ...
    response.status.should == 200
    json.has_key?('errors').should be_false

    json = api_post 'user', {user: {email: 'user2@example.com', password: '123'}}
    response.status.should == 422
    json['errors']['password'].should include('is too short (minimum is 6 characters)')
  end
  ...
  it 'should change user\'s email and password' do
    ...
    response.status.should == 200
    json['email'].should == 'new@sample.com'
  ...
  it 'should completely remove user and all his integrations/keys' do
    ...
    api_delete 'user', {token: @user.api_key.token}
